...
private func convertToProjectAttributes(_ dictionary: [String: Any]) -> [String: ProjectAttribute] {
    var result: [String: ProjectAttribute] = [:]
...
public class PBXProjGenerator {
    ...
    public func generate() throws -> PBXProj {
        ...
        let defaultAttributes: [String: ProjectAttribute] = [
            "BuildIndependentTargetsInParallel": "YES"
        ]
        var projectAttributes: [String: ProjectAttribute] = defaultAttributes.merged(convertToProjectAttributes(project.attributes))
        
        // Set default LastUpgradeCheck if user did not specify
        let lastUpgradeKey = "LastUpgradeCheck"
        if projectAttributes[lastUpgradeKey]?.stringValue == nil {
    ...
    func generateTargetAttributes() -> [PBXTarget: [String: ProjectAttribute]] {
        ...
        func generateTargetAttributes(_ target: ProjectTarget, pbxTarget: PBXTarget) {
            if !target.attributes.isEmpty {
                targetAttributes[pbxTarget, default: [:]].merge(convertToProjectAttributes(target.attributes))
            }
    ...
    func generateTarget(_ target: Target) throws {
        ...
        var copyPhaseMapping: [ObjectIdentifier: BuildPhaseSpec.CopyFilesSettings] = [:]
        ...
        func processTargetDependency(_ dependency: Dependency, dependencyTarget: Target, embedFileReference: PBXFileElement?, platform: String?, platforms: [String]?) {
            ...
            if embed {
                ...
                if let copyPhase = dependency.copyPhase {
                    copyPhaseMapping[ObjectIdentifier(embedFile)] = copyPhase
        ...
        func splitCopyDepsByDestination(_ references: [PBXBuildFile]) -> [BuildPhaseSpec.CopyFilesSettings : [PBXBuildFile]] {
            ...
            for reference in references {
                ...
                guard let key = copyPhaseMapping[ObjectIdentifier(reference)] else { continue }
