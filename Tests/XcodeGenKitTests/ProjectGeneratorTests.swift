    ...
    func testTargets() throws {
        ...
        describe {
            ...
            $0.it("generates target attributes") {
                ...
                guard case .targetReference(let testTargetRef) = targetAttributes[uiTestTarget]?["TestTargetID"] else {
                    throw failure("TestTargetID is not a target reference")
                }
                try expect((testTargetRef as? PBXNativeTarget)?.name) == app.name
